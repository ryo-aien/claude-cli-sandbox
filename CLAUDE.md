...
Claude に読み込ませる必要のないファイルやディレクトリは、プロジェクトルートの `.gitignore` に一括で追加してコンテキストから除外すること。以下は除外すべき代表的な対象：

- `node_modules/`、`vendor/`、`.venv/` などの依存関係ディレクトリ
- `dist/`、`build/`、`out/` などのビルド成果物
- `__pycache__/`、`*.pyc` などのキャッシュファイル
- ログファイル、一時ファイル
- 大量の画像・バイナリファイル

除外ルールはプロジェクトルートの `.gitignore` で一元管理し、ディレクトリごとに `.gitignore` を分散させない。プロジェクトのファイル数が多い場合、不要なファイルを `.gitignore` に含めることでトークン消費を大幅に削減できる。
