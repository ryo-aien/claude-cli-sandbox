...
| `/terminal-setup` | Shift+Enter などのターミナルキーバインドを設定（必要なターミナルのみ表示） |
...
| コマンド | 説明 |
|---|---|
| `/diff` | 未コミット変更とターンごとの差分をインタラクティブに表示 |
| `/security-review` | 現ブランチの変更をセキュリティ脆弱性の観点で分析 |
| `/pr-comments [PR]` | GitHub PRのコメントを取得・表示（`gh` CLI 必須） |
...
| `/memory` | `CLAUDE.md` ファイルを編集、オートメモリの管理 |
...
| コマンド | 説明 |
|---|---|
| `/insights` | セッション分析レポートを生成（プロジェクト領域、インタラクションパターンなど） |
...
| `/feedback [report]` | フィードバックを送信。エイリアス: `/bug` |
...
| `/mobile` | Claude モバイルアプリの QR コードを表示。エイリアス: `/ios`, `/android` |
| `/statusline` | Claude Code のステータスラインを設定 |
| `/remote-control` | このセッションを claude.ai からリモートコントロール対象にする。エイリアス: `/rc` |
| `/remote-env` | `--remote` 起動時のデフォルトリモート環境を設定 |
...
ビルトインコマンドとは異なり、プロンプトベースで動作します。ファイル読み込みや並列エージェント起動も可能。

| スキル | 説明 |
|---|---|
| `/batch <instruction>` | コードベース全体に大規模変更を並列実行。5〜30の独立ユニットに分解し、git worktree で並列エージェントを起動。各エージェントが実装・テスト・PRを作成。git リポジトリ必須。例: `/batch migrate src/ from Solid to React` |
| `/claude-api` | プロジェクトの言語に応じた Claude API リファレンスを読み込む（Python, TypeScript, Java, Go, Ruby, C#, PHP, cURL）。`anthropic` などのインポートがあると自動起動 |
| `/debug [description]` | 現在のセッションのデバッグログを読んでトラブルシューティング |
| `/loop [interval] <prompt>` | 指定間隔でプロンプトを繰り返し実行。デプロイ監視や PR の定期チェックなどに。例: `/loop 5m check if the deploy finished` |
| `/simplify [focus]` | 最近変更したファイルのコード品質・効率を並列3エージェントでレビューして修正。例: `/simplify focus on memory efficiency` |
